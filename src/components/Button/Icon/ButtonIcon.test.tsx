...
describe('<ButtonIcon />', () => {
  ...
  it('should add the className in button component', () => {
    const {container} = render(<ButtonIcon className="customClass" icon="check" />)
    expect(container.querySelector('.customClass')).toBeInstanceOf(HTMLButtonElement)
  })

  it('should receive the isDisabled attribute', () => {
    const {container} = render(<ButtonIcon className="customClass" icon="check" isDisabled />)
  ...
  it('should receive the onChange attribute', () => {
    ...
    const {container} = render(<ButtonIcon icon="check" onClick={handleClick} />)
